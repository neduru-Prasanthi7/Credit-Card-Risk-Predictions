...
class CREDIT_CARD_DATA:
    def __init__(self,path):
        try:
            ...
            logger.info(f'total no of X_train columns:{self.X_train.columns}')
            logger.info(f'total no of X_test columns :{self.X_test.columns}')

            logger.info(f'{self.X_train.sample(4)}')
            logger.info(f'{self.X_test.sample(4)}')
    ...
    def vt_hol(self):
        try:
            ...
            logger.info(f'{self.X_train_num.isnull().sum()} --> {self.X_test_num.isnull().sum()}')
...
if __name__ == "__main__":
    try:
            obj=CREDIT_CARD_DATA(f'C:\\Users\\nedur\\Downloads\\Ml_pipeline\\creditcard.csv')
            obj.missing_values()
            obj.vt_hol()
            obj.fs()
            obj.cat_to_num()
